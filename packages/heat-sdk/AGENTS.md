...
- Do not weaken `isSensitiveInput`, selector blocking, masked input behavior, or metadata sanitization without an explicit product decision.
- Preserve listener cleanup and history restoration in `shutdown`.
- Keep queued events, localStorage persistence, retry/backoff, and flush semantics stable unless the task targets them.
